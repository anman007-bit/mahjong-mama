...
def _build_turtle_layout():
    """Раскладка ЧЕРЕПАХА: 152 плитки = 76 пар = 38 четвёрок.
    Размер сетки: 17 колонок (0-16) × 7 рядов (0-6).
    Слой 0 (основа): 77 плиток
    Слой 1: 50 плиток
    Слой 2: 23 плитки
    Слой 3: 2 плитки (макушка)
    """
    layout = []

    def add_row(layer, row, pattern):
        """Добавить ряд плиток из строки '.' (пусто) и 'X' (плитка)."""
        for col, ch in enumerate(pattern):
            if ch == 'X':
                layout.append((layer, row, col))

    # ===== СЛОЙ 0 (основа, 77 плиток) =====
    #              0123456789012345 6
    add_row(0, 0, '.XXX..XXX..XXX...')   # лапы передние + макушка панциря
    add_row(0, 1, '...XXXXXXXX......')   # тело
    add_row(0, 2, '..XXXXXXXXXX.XXX.')   # тело + голова (с шеей-разрывом)
    add_row(0, 3, 'XXXXXXXXXXXXXXXXX')   # хвост + тело + голова (вся ось)
    add_row(0, 4, '..XXXXXXXXXX.XXX.')   # тело + голова
    add_row(0, 5, '...XXXXXXXX......')   # тело
    add_row(0, 6, '.XXX..XXX..XXX...')   # лапы задние + донышко панциря

    # ===== СЛОЙ 1 (50 плиток) =====
    #              0123456789012345 6
    add_row(1, 0, '..XX.......XX....')   # по 2 на каждой передней лапе
    add_row(1, 1, '....XXXXXX.......')   # овал на панцире
    add_row(1, 2, '...XXXXXXXX...X..')   # овал + ухо ромба головы
    add_row(1, 3, '.XX.XXXXXXX..XXX.')   # хвост + овал + центр ромба
    add_row(1, 4, '...XXXXXXXX...X..')   # овал + ухо ромба
    add_row(1, 5, '....XXXXXX.......')   # овал
    add_row(1, 6, '..XX.......XX....')   # по 2 на каждой задней лапе

    # ===== СЛОЙ 2 (23 плитки) =====
    add_row(2, 0, '...X.......X.....')   # по 1 жёлтой на передних лапах
    add_row(2, 2, '....XXXXXX.......')   # жёлтый овал на панцире
    add_row(2, 3, '....XXXXXX....X..')   # жёлтый овал + центр головы
    add_row(2, 4, '....XXXXXX.......')   # жёлтый овал
    add_row(2, 6, '...X.......X.....')   # по 1 жёлтой на задних лапах

    # ===== СЛОЙ 3 (2 плитки - макушка) =====
    add_row(3, 3, '......XX.........')   # 2 красные на самой вершине
...
def _build_diamond_layout():
    ...
    # ===== СЛОЙ 1 =====
    # Ромб поменьше (на 2 плитки уже с каждой стороны), сдвинут на +1 чтобы стоял ровно над слоем 0
    diamond_rows_l1 = [
        (1, range(7, 10)),    # 3 плитки
        (2, range(6, 11)),    # 5 плиток
        (3, range(5, 12)),    # 7 плиток (середина)
        (4, range(6, 11)),    # 5 плиток
        (5, range(7, 10)),    # 3 плитки
    ]
    for row, cols in diamond_rows_l1:
        for col in cols:
            layout.append((1, row, col))

    # Углы слоя 1 - по 1 плитке над средней плиткой каждого угла (углы НЕ трогаем, они и так правильно)
...
# ============================================================
# РАСКЛАДКА "КРЕПОСТЬ С ЗАМКОМ" (108 плиток)
# ============================================================

def _build_fortress_layout():
    """Раскладка КРЕПОСТЬ: 102 плитки = 51 пара.
    Стены с воротами + зубцы по углам + замок внутри + башня + макушка.
    Слой 0: 42 (стены крепости с 3 воротами в верхней и нижней)
    Слой 1: 39 (зубцы по углам + замок)
    Слой 2: 15 (центральная часть замка)
    Слой 3: 5 (башня крестом)
    Слой 4: 1 (макушка)
    """
    layout = []

    def add_row(layer, row, pattern):
        """Добавить ряд из строки '.' (пусто) и 'X' (плитка)."""
        for col, ch in enumerate(pattern):
            if ch == 'X':
                layout.append((layer, row, col))

    # ===== СЛОЙ 0 - стены крепости с воротами (42 плитки) =====
    # Разрывы (ворота) в стенах ряда 0 и 6 на col 4, 8, 12
    # делают игру значительно проще — плитки внутри стены освобождаются с обеих сторон
    #              0123456789012345 6
    add_row(0, 0, 'XXXX.XXX.XXX.XXXX')   # верхняя стена с 3 воротами (13)
    add_row(0, 1, 'XX.............XX')   # угловые башни сверху (4)
    add_row(0, 2, 'X...............X')   # боковые стены (2)
    add_row(0, 3, 'X...............X')   # боковые стены (2)
    add_row(0, 4, 'X...............X')   # боковые стены (2)
    add_row(0, 5, 'XX.............XX')   # угловые башни снизу (4)
    add_row(0, 6, 'XXXX.XXX.XXX.XXXX')   # нижняя стена с 3 воротами (13)

    # ===== СЛОЙ 1 - зубцы по углам + замок (39 плиток) =====
    add_row(1, 0, '.X.X.........X.X.')   # зубцы по углам сверху (4)
    add_row(1, 1, '......XXXXX......')   # крыша замка (5)
    add_row(1, 2, '.....XXXXXXX.....')   # замок (7)
    add_row(1, 3, '.....XXXXXXX.....')   # замок (7)
    add_row(1, 4, '.....XXXXXXX.....')   # замок (7)
    add_row(1, 5, '......XXXXX......')   # крыша замка снизу (5)
    add_row(1, 6, '.X.X.........X.X.')   # зубцы по углам снизу (4)

    # ===== СЛОЙ 2 - центр замка (15 плиток) =====
    add_row(2, 2, '......XXXXX......')   # 5
    add_row(2, 3, '......XXXXX......')   # 5
    add_row(2, 4, '......XXXXX......')   # 5

    # ===== СЛОЙ 3 - башня крестом (5 плиток) =====
    add_row(3, 2, '........X........')   # 1 - верх креста
    add_row(3, 3, '.......XXX.......')   # 3 - середина креста
    add_row(3, 4, '........X........')   # 1 - низ креста

    # ===== СЛОЙ 4 - макушка (1 плитка) =====
    add_row(4, 3, '........X........')   # 1

    return layout


FORTRESS_SHAPE = Shape(
    key='fortress',
    name='КРЕПОСТЬ',
    layout=_build_fortress_layout(),
    board_w=17,
    board_h=7
)
...
SHAPES = [PYRAMID_SHAPE, DIAMOND_SHAPE, FORTRESS_SHAPE, TURTLE_SHAPE]
